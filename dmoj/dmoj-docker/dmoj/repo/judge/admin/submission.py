...
class SubmissionAnalysisAdmin(admin.ModelAdmin):
    list_display = (
        'analysis_id', 'submission_link', 'problem_code', 
        'user_column', 'sae_score', 'final_score', 'analysis_time'
    )
    list_display_links = ('analysis_id',)
    list_filter = ('analysis_time',)
    search_fields = ('submission__id', 'submission__problem__code', 'submission__user__user__username')
    
    fields = ('submission', 'sae_score', 'final_score', 'analysis_time', 'dynamic_metrics_view')
    readonly_fields = ('submission', 'sae_score', 'final_score', 'analysis_time', 'dynamic_metrics_view')
    ...
    @admin.display(description=_('Analysis ID'), ordering='submission_id')
    def analysis_id(self, obj):
        return format_html('<b> {} #{}</b>', gettext('Analysis'), obj.submission_id)

    @admin.display(description=_('Submission'), ordering='submission_id')
    def submission_link(self, obj):
        url = reverse('admin:judge_submission_change', args=[obj.submission_id])
        return format_html('<a href="{}" target="_blank">{} #{} ↗</a>', url, gettext('Submission'), obj.submission_id)

    @admin.display(description=_('Problem'), ordering='submission__problem__code')
    def problem_code(self, obj):
        return obj.submission.problem.code

    @admin.display(description=_('User'), ordering='submission__user__user__username')
    def user_column(self, obj):
        return obj.submission.user.user.username

    @admin.display(description=_('Calculated Metrics (SAE)'))
    def dynamic_metrics_view(self, obj):


        current_lang = (get_language() or 'en')[:2]

        
        localized_metrics = obj.get_localized_metrics(lang_code=current_lang)

        
        if not localized_metrics:
            return format_html('<span style="color: gray; font-style: italic;">{}</span>', gettext('No data'))
        ...
        for metric in localized_metrics:
            ...
            if key == 'syntax_errors' and val > 0:
                val_html = format_html('<span style="color: #dc3545; font-weight: bold;">{}</span>', val)
            elif isinstance(val, float):
                val_html = format_html('<b>{}</b>', f'{val:.2f}')
            else:
                val_html = format_html('<b>{}</b>', val)
                
            items_html.append(format_html('<li>{}: {}</li>', metric['name'], val_html))
        
        return format_html(
            '<ul style="margin: 0; padding-left: 20px; line-height: 2.0; font-size: 13px;">{}</ul>',
            mark_safe(''.join(items_html))
        )