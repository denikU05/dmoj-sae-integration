...
class ProblemForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        user_lang = (get_language() or 'en')[:2]
        if user_lang not in {'ru', 'en'}:
            user_lang = 'en'

        dir_label = "Направление" if user_lang == 'ru' else "Direction"
        weight_label = "Вес" if user_lang == 'ru' else "Weight"
        dir_choices = (
            [('lower_better', 'Меньше — лучше'), ('higher_better', 'Больше — лучше')]
            if user_lang == 'ru'
            else [('lower_better', 'Lower is better'), ('higher_better', 'Higher is better')]
        )

        config = getattr(self.instance, 'sae_metrics_config', {}) if getattr(self, 'instance', None) and self.instance.pk else {}

        for key, translations in METRIC_TRANSLATIONS.items():
            base_field = f'sae_{key}'
            
            if f'{base_field}_enabled' not in self.fields:
                continue

            metric_name = translations.get(user_lang, translations.get('en', key))
            
            self.fields[f'{base_field}_enabled'].label = metric_name
            self.fields[f'{base_field}_direction'].label = dir_label
            self.fields[f'{base_field}_direction'].choices = dir_choices
            self.fields[f'{base_field}_weight'].label = weight_label

            metric_config = config.get(key, {})
            self.initial[f'{base_field}_enabled'] = metric_config.get('enabled', False)
            self.initial[f'{base_field}_direction'] = metric_config.get('direction', 'lower_better')
            self.initial[f'{base_field}_weight'] = metric_config.get('weight', 1.0)

    def clean(self) -> dict:
        cleaned_data = super().clean()
        sae_metrics_config = {}

        for key in METRIC_TRANSLATIONS:
            base_field = f'sae_{key}'
            if f'{base_field}_enabled' in cleaned_data:
                sae_metrics_config[key] = {
                    'enabled': cleaned_data.get(f'{base_field}_enabled', False),
                    'direction': cleaned_data.get(f'{base_field}_direction', 'lower_better'),
                    'weight': cleaned_data.get(f'{base_field}_weight', 1.0),
                }

        cleaned_data['sae_metrics_config'] = sae_metrics_config
        return cleaned_data

    def save(self, commit: bool = True):
        instance = super().save(commit=False)
        
        if 'sae_metrics_config' in self.cleaned_data:
            instance.sae_metrics_config = self.cleaned_data['sae_metrics_config']
    ...
    class Meta:
        widgets = {
            ...
            'sae_max_penalty': forms.NumberInput(attrs={'min': 0, 'max': 100, 'style': 'width: 100px;'}),
        }

for key in METRIC_TRANSLATIONS.keys():
    ProblemForm.base_fields[f'sae_{key}_enabled'] = forms.BooleanField(required=False)
    ProblemForm.base_fields[f'sae_{key}_direction'] = forms.ChoiceField(choices=[], required=False)
    ProblemForm.base_fields[f'sae_{key}_weight'] = forms.FloatField(required=False, min_value=0.0)
...
class ProblemAdmin(NoBatchDeleteMixin, VersionAdmin):
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        for key, data in METRIC_TRANSLATIONS.items():
            metric_name = data.get('en', key)
            base_field = f'sae_{key}'

            form.base_fields[f'{base_field}_enabled'] = forms.BooleanField(
                label=f'[{metric_name}] Enable', 
                required=False
            )
            form.base_fields[f'{base_field}_direction'] = forms.ChoiceField(
                label=f'[{metric_name}] Direction',
                choices=[('lower_better', 'Lower is better'), ('higher_better', 'Higher is better')],
                required=False
            )
            form.base_fields[f'{base_field}_weight'] = forms.FloatField(
                label=f'[{metric_name}] Weight', 
                required=False, 
                min_value=0.0
            )
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        sae_fields = ['sae_max_penalty']
        for key in METRIC_TRANSLATIONS:
            base_field = f'sae_{key}'
            sae_fields.append((f'{base_field}_enabled', f'{base_field}_direction', f'{base_field}_weight'))

        user_lang = getattr(request, 'LANGUAGE_CODE', 'en')[:2]
        title = 'Настройки статического анализа (SAE)' if user_lang == 'ru' else 'Static Analysis Settings (SAE)'

        fieldsets.append((title, {'fields': sae_fields}))
        return fieldsets
